...
PHP_METHOD(tarantool_class, authenticate) {
	...
	zval *id;
	TARANTOOL_PARSE_PARAMS(id, "z|s", &login_zval, &passwd, &passwd_len);
	/* need checking that type of zval is string */
	if (Z_TYPE(login_zval) != IS_STRING) {
		THROW_EXC("Tarantool::authenticate expects parameter 1 to be string");
		RETURN_NULL();
	}
	login = Z_STRVAL_P(login_zval);
	login_len = Z_STRLEN_P(login_zval);
